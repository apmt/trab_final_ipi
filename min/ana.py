...
def binarization (img):
	os.system("mindtct nova.png oroot/oroot")
	img_aux = img.copy()
	h, w = img.shape
	brw_file = open("oroot/oroot.brw", "rb")
	for y in range (0, h):
		for x in range (0, w):
...
img2 = average_img(img_kern_3, img_kern_5, img_kern_7, img_kern_9)
#img2 = cv2.adaptiveThreshold(img,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 51, 20)

cv2.imshow('image',img2)
cv2.imwrite("nova.png", img2)

img = binarization(img2)
cv2.imshow('imag',img)
cv2.imwrite("bi.png", img)
